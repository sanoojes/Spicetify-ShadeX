...
function Write-HelloMessage {
    ...
    process {
        ...
        Write-Center -Message 'Starting the Spicetify ShadeX script...' -ForegroundColor Blue
        Write-Host
        Write-Center -Message 'github.com/sanoojes/Spicetify-ShadeX' -ForegroundColor Blue
        Write-Center -Message '----------------------------------------' -ForegroundColor Blue
        Write-Host
    }
}
...
function Write-ByeMessage {
    ...
    process {
        ...
        Write-Host
        Write-Center -Message 'Thanks for using ShadeX!' -ForegroundColor Green
        Write-Center -Message '----------------------------------------' -ForegroundColor Green
        Write-Host
    }
}
...
function Get-SpicetifyFoldersPaths {
    ...
    process {
        @{
            ...
            ShadeXPath  = "$(spicetify path userdata)\Themes\ShadeX"
        }
    }
}
...
function Get-ShadeX {
    ...
    param ()
    begin {
        $archiveName = 'ShadeX-main'
        $Temp = [System.IO.Path]::GetTempPath()
        $archivePath = "$Temp\$archiveName.zip"
    }
    process {
        Write-Verbose -Message 'Downloading the ShadeX repository archive...' -Verbose
        $Parameters = @{
            UseBasicParsing = $true
            Uri             = 'https://codeload.github.com/sanoojes/Spicetify-ShadeX/zip/refs/heads/main'
            OutFile         = $archivePath
        }
        Invoke-WebRequest @Parameters
        
        Write-Verbose -Message 'Unpacking the ShadeX repository archive...' -Verbose
        $Parameters = @{
            Path            = $archivePath
...
function Install-ShadeX {
    ...
    begin {
        Write-Verbose -Message 'Installing ShadeX theme...' -Verbose
        $ShadeXSrcPath = "$Path\src"
        $ShadeXRemotePath = "$Path\remote"
    }
    process {
...
function Uninstall-ShadeX {
    ...
    begin {
        Write-Verbose -Message 'Uninstalling ShadeX theme...' -Verbose
    }
    process {
